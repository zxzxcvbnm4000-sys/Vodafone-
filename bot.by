...
# قاموس مؤقت لحفظ بيانات كل مستخدم أثناء خطوات الإدخال
user_data = {}
...
def change_pass(number, token, password, newPass):
  ...
  payload = {
      ...
      "customerAccount": {
          "authentication": {"password": password, "newPassword": newPass}
      },
...
def send_welcome(message):
  ...
  user_data[chat_id] = {}
  bot.send_message(chat_id, "مرحباً بك! يرجى إدخال رقم الهاتف:")
...
def get_number(message):
  ...
  bot.send_message(chat_id, "تمام، الآن قم بإدخال كلمة المرور الحالية:")
...
def get_password(message):
  ...
  bot.send_message(chat_id, "أخيراً، قم بإدخال كلمة المرور الجديدة:")
  bot.register_next_step_handler(message, get_new_password)


def get_new_password(message):
  chat_id = message.chat.id
  user_data[chat_id]["newPass"] = message.text.strip()

  number = user_data[chat_id]["number"]
  password = user_data[chat_id]["password"]
  newPass = user_data[chat_id]["newPass"]

  bot.send_message(chat_id, "⏳ جاري تنفيذ الطلب والتواصل مع الخادم...")
  ...
  try:
    ...
    if data.get("reason") == "Repeated Password":
      bot.send_message(
          chat_id,
          "❌ فشل التغيير: كلمة المرور الجديدة مستخدمة من قبل. استخدم كلمة مرور"
          " أخرى.",
      )
    elif data.get("state") == "updated":
      bot.send_message(chat_id, "✅ تم تغيير كلمة المرور بنجاح!")
    else:
      bot.send_message(chat_id, f"⚠️ استجابة غير متوقعة:
{data}")
